...
apt install --yes git python3-pip linux-headers-5.19.0-38-generic build-essential python3-dev xvfb appstream tar lsb-release apt-utils file upx > /dev/null
...
py_deps_DDoS=$py_deps_DDoS' --collect-all DDoS.default_config --collect-all DDoS.default_redeems --collect-all DDoS.sqlite'
